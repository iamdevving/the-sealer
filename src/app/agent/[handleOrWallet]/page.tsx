...
export default function AgentProfilePage() {
  ...
  const entityColor = profile?.sid?.entityType === 'AI_AGENT' ? accent
    : profile?.sid?.entityType === 'HUMAN' ? '#9ca3af' : '#f59e0b';
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .avatar {
          ...
          background: ${faint}; flex-shrink: 0; overflow: hidden;
          border: 0.8px solid ${faint};
          display: flex; align-items: center; justify-content: center;
        }
        .avatar img { width: 100%; height: 100%; object-fit: cover; }
        .avatar-placeholder svg { opacity: 0.3; }
        .header-info { flex: 1; }
        .header-handle { font-size: 20px; color: ${accent}; letter-spacing: 1px; margin-bottom: 4px; }
        .header-name { font-family: Georgia, serif; font-size: 14px; color: ${ink}; margin-bottom: 8px; }
        .header-wallet { font-size: 8px; color: ${inkDim}; letter-spacing: 0.5px; margin-bottom: 10px; }
        ...
        .commitment-right { display: flex; flex-direction: column; align-items: flex-end; gap: 4px; }
        .commitment-pts { font-size: 13px; font-weight: 700; }
        .commitment-status { font-size: 7px; letter-spacing: 0.5px; }
        .commitment-diff {
          width: 26px; height: 26px; border-radius: 50%;
          display: flex; align-items: center; justify-content: center;
          border: 1px solid ${faint}; font-size: 9px; font-weight: 700;
          color: ${inkDim};
        }
              ...
              <div className="header-inner">
                <div className="avatar">
                  {profile.sid?.imageUrl ? (
                    ...
                    <img src={profile.sid.imageUrl} alt={profile.sid.name}/>
                  ) : (
                    <div className="avatar-placeholder">
                      <svg width="40" height="40" viewBox="0 0 60 60" fill="none">
                        <circle cx="30" cy="30" r="28" stroke="#5a7090" strokeWidth="1.5"/>
                        <circle cx="30" cy="24" r="10" fill="#5a7090" opacity="0.4"/>
                        <ellipse cx="30" cy="46" rx="16" ry="10" fill="#5a7090" opacity="0.4"/>
                      </svg>
                    </div>
                  )}
                </div>
                ...
                <div className="header-info">
                  {profile.handle
                    ...
                    : <div className="header-handle" style={{opacity:0.5}}>{truncate(profile.wallet)}</div>
                  }
                  {profile.sid?.name && (
                    <div className="header-name">{profile.sid.name}</div>
                  )}
                  <div className="header-wallet">{profile.wallet}</div>
                  <div className="header-pills">
                    {profile.sid?.entityType && (
                      <div className="pill" style={{background:`${entityColor}18`, border:`0.8px solid ${entityColor}80`, color: entityColor}}>
                        {profile.sid.entityType === 'AI_AGENT' ? 'AI AGENT' : profile.sid.entityType}
                      </div>
                    )}
                    {profile.sid?.chain && (
                ...
                <div className="header-stats">
                  ...
                  <div className="stat-lbl" style={{marginTop:4}}>{profile.achievementCount} ACHIEVEMENT{profile.achievementCount !== 1 ? 'S' : ''}</div>
                  {profile.rank && (
                  ...
                  <div key={i} className="commitment-row">
                    ...
                    <div className="commitment-info">
                      ...
                      <div className="commitment-statement">
                        {c.statement?.slice(0, 80)}{c.statement?.length > 80 ? '…' : ''}
                      </div>
                      {c.deadline && (
