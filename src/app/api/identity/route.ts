...
function buildStamp(stampColor: string): string {
  ...
  return '<circle cx="0" cy="0" r="' + r + '" fill="none" stroke="' + stampColor + '" stroke-width="2.5"/>' +
    ...
    '</defs>' +
    '<text font-family="monospace" font-size="7.5" letter-spacing="2.8" fill="' + stampColor + '"><textPath href="#topRim" startOffset="6%">THESEALER.XYZ</textPath></text>' +
    '<text font-family="monospace" font-size="7.5" letter-spacing="2" fill="' + stampColor + '"><textPath href="#botRim" startOffset="14%">SEALER ID</textPath></text>' +
    '';
}
...
export async function GET(req: NextRequest) {
  ...
  // FIXED: Added esc() to ensure comma-separated values don't break XML
  ...
  if (imageUrl) {
    try {
      ...
      if (res.ok) {
        ...
        // FIXED: Safely split the content-type string to prevent attribute injection
  ...
  // FIXED: Replace raw '<' with safe XML entity '&lt;'
  ...
  // FIXED: Using safeMrz variables at the bottom of the SVG template
  const svg = '<svg xmlns="http://www.w3.org/2000/svg" width="' + W + '" height="' + H + '" viewBox="0 0 ' + W + ' ' + H + '">' +
    ...
    '<g transform="translate(' + (W - PAD - 60) + ',' + (DIV2_Y + 72) + ') rotate(-12)">' + stamp + '</g>' +
