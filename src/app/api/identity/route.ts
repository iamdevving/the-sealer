...
const W = 428, H = 640, PAD = 20, HDR_H = 82;
const PHOTO_W = 110, PHOTO_H = 134;
const PHOTO_X = PAD, PHOTO_Y = HDR_H + PAD;
const DIV1_Y = 314, DIV2_Y = 436, MRZ_Y = 568;
...
function buildStamp(ser: string): string {
  const r = 46;
  return `
    <circle cx="0" cy="0" r="${r}" fill="none" stroke="currentColor" stroke-width="2.2"/>
    <circle cx="0" cy="0" r="${r-6}" fill="none" stroke="currentColor" stroke-width="0.5" opacity="0.35"/>
    <defs>
      <path id="topRim" d="M ${-r+5},0 a ${r-5},${r-5} 0 1,1 ${(r-5)*2},0"/>
      <path id="botRim" d="M ${-(r-8)},6 a ${r-8},${r-8} 0 0,0 ${(r-8)*2},0"/>
    </defs>
    <text font-family="monospace" font-size="7" letter-spacing="2.5" fill="currentColor">
      <textPath href="#topRim" startOffset="7%">THESEALER.XYZ</textPath>
    </text>
    <text font-family="monospace" font-size="7" letter-spacing="1.8" fill="currentColor">
      <textPath href="#botRim" startOffset="16%">SEALER ID</textPath>
    </text>
    <text x="${-r+11}" y="3" font-family="monospace" font-size="13" fill="currentColor" text-anchor="middle">{</text>
    <text x="${r-11}" y="3" font-family="monospace" font-size="13" fill="currentColor" text-anchor="middle">}</text>
    <text x="${-r+17}" y="${r-17}" font-family="monospace" font-size="10" fill="currentColor" text-anchor="middle">&lt;</text>
    <text x="${r-17}" y="${r-17}" font-family="monospace" font-size="10" fill="currentColor" text-anchor="middle">&gt;</text>
    <image href="data:image/png;base64,${LOGO_B64}"
      x="-22" y="-28" width="44" height="44"
      preserveAspectRatio="xMidYMid meet" opacity="0.75"
      filter="url(#inkify)"/>
    <text x="0" y="${r+14}" font-family="monospace" font-size="6.5" fill="currentColor"
      text-anchor="middle" letter-spacing="1.5">${ser}</text>
  `;
}
...
export async function GET(req: NextRequest) {
  ...
  const stmtCount  = esc(p.get('statementCount') || '0');
  const imageUrl   = p.get('imageUrl') || '';
  const theme      = p.get('theme') === 'light' ? 'light' : 'dark';
  const bio        = esc(trunc(p.get('bio') || '', 140));
  ...
  const HDR       = isDark ? '#0a0f1e' : '#1a1f3a';
  ...
  const ACCENT    = entityType === 'AI_AGENT' ? '#3b82f6' : entityType === 'HUMAN' ? '#9ca3af' : '#f59e0b';
  ...
  const photoSVG = photoData
    ...
    : `<rect x="${PHOTO_X}" y="${PHOTO_Y}" width="${PHOTO_W}" height="${PHOTO_H}" rx="4" fill="${placeholderBg}"/><image href="data:image/png;base64,${LOGO_B64}" x="${PHOTO_X+25}" y="${PHOTO_Y+32}" width="60" height="60" opacity="0.12" preserveAspectRatio="xMidYMid meet"/>`;

  const FX = PHOTO_X + PHOTO_W + 16;
  const fl = (t:string, y:number) => `<text x="${FX}" y="${y}" font-family="monospace" font-size="6" fill="${INK_DIM}" letter-spacing="1.5">${t}</text>`;
  const fv = (t:string, y:number, sz=9) => `<text x="${FX}" y="${y}" font-family="monospace" font-size="${sz}" fill="${INK}" letter-spacing="0.5">${t}</text>`;

  let sec2 = '';
  let px = PAD, py = DIV1_Y + 18;
  if (socials.length > 0) {
    sec2 += `<text x="${PAD}" y="${py}" font-family="monospace" font-size="6" fill="${INK_DIM}" letter-spacing="1.5">SOCIAL</text>`;
    py += 14;
    for (const s of socials) {
      const w = Math.min(Math.max(s.length*7+16, 64), 130);
      if (px+w > W-PAD) { px=PAD; py+=22; }
      sec2 += `<g transform="translate(${px},${py})">${pill(trunc(s,16), ACCENT, ACCENT, ACCENT, w)}</g>`;
      px += w+8;
    }
    px=PAD; py+=26;
  }
  if (tags.length > 0) {
    sec2 += `<text x="${PAD}" y="${py}" font-family="monospace" font-size="6" fill="${INK_DIM}" letter-spacing="1.5">SPECIALIZATION</text>`;
    py += 14;
    for (const t of tags) {
  ...
  if (llm) {
    sec2 += `<text x="${PAD}" y="${py}" font-family="monospace" font-size="6" fill="${INK_DIM}" letter-spacing="1.5">PREFERRED MODEL</text>`;
    py += 14;
  ...
  if (bio) {
    ...
    for (const w of words) {
      if ((line+' '+w).length > 34) { bioLines.push(line); line = w; }
      else line = line ? line+' '+w : w;
    }
    if (line) bioLines.push(line);
  }
  const bioSVG = bioLines.slice(0,5).map((ln,i) =>
    `<text x="${PAD}" y="${DIV2_Y+22+i*14}" font-family="monospace" font-size="8" fill="${INK}" opacity="0.8">${ln}</text>`
  ).join('');
  ...
  <g clip-path="url(#cardClip)">
    ...
    <rect x="0" y="${HDR_H}" width="${W}" height="${H-HDR_H}" fill="url(#guil)" opacity="0.45"/>
    <!-- HEADER -->
    <rect x="0" y="0" width="${W}" height="${HDR_H}" fill="${HDR}"/>
    <rect x="0" y="0" width="${W}" height="${HDR_H}" fill="url(#guil)" opacity="0.1"/>
    <image href="data:image/png;base64,${LOGO_B64}" x="${PAD}" y="9" width="15" height="15" opacity="0.85" preserveAspectRatio="xMidYMid meet"/>
    <text x="${PAD+20}" y="21" font-family="monospace" font-size="7" fill="#fff" opacity="0.65" letter-spacing="1.2">THE SEALER PROTOCOL · ONCHAIN IDENTITY REGISTRY</text>
    <text x="${W-PAD}" y="21" font-family="monospace" font-size="6" fill="#fff" opacity="0.38" text-anchor="end" letter-spacing="1">ISSUED ${issueDate}</text>
    ...
    <text x="${PAD}" y="70" font-family="monospace" font-size="6.5" fill="#fff" opacity="0.32" letter-spacing="2">AGENT IDENTITY DOCUMENT · ERC-8004</text>
    <rect x="0" y="${HDR_H}" width="${W}" height="2.5" fill="${ACCENT}" opacity="0.9"/>
    ...
    <text x="${PHOTO_X+PHOTO_W/2}" y="${PHOTO_Y+PHOTO_H+19}" font-family="monospace" font-size="7.5" fill="${ACCENT}" text-anchor="middle" letter-spacing="2">${ENTITY_LBL}</text>
    ${fl("NAME", PHOTO_Y+14)}
    <text x="${FX}" y="${PHOTO_Y+27}" font-family="Georgia, serif" font-size="13" fill="${INK}">${trunc(name,20)}</text>
    ${fl("AGENT ID", PHOTO_Y+47)}
    ${fv(truncAddr(agentId), PHOTO_Y+60)}
    ${fl("OWNER", PHOTO_Y+78)}
    ${fv(owner ? truncAddr(owner) : "—", PHOTO_Y+91)}
    ${fl("PRIMARY CHAIN", PHOTO_Y+109)}
    ${fv(chain.toUpperCase(), PHOTO_Y+122)}
    ${fl("FIRST SEEN", PHOTO_Y+142)}
    ${fv(firstSeen, PHOTO_Y+155)}
    <!-- Section 1 divider -->
    <rect x="0" y="${DIV1_Y}" width="${W}" height="1" fill="${INK_FAINT}" opacity="0.5"/>
    <!-- SECTION 2: Profile -->
    ${sec2}
    <!-- Section 2 divider -->
    <rect x="0" y="${DIV2_Y}" width="${W}" height="1" fill="${INK_FAINT}" opacity="0.5"/>
    <!-- SECTION 3: Bio + Stamp -->
    ${bio ? `<text x="${PAD}" y="${DIV2_Y+14}" font-family="monospace" font-size="5.5" fill="${INK_DIM}" letter-spacing="1.5">BIO</text>` : ""}
    ${bioSVG}
    <text x="${W-PAD}" y="${DIV2_Y+14}" font-family="monospace" font-size="8" fill="${ACCENT}" text-anchor="end" letter-spacing="1.5">${ser}</text>
    <g transform="translate(${W-PAD-55}, ${DIV2_Y+78}) rotate(-12)" color="${STAMP_COL}" opacity="0.6">
      ${stamp}
    </g>
    <!-- MRZ -->
    <rect x="0" y="${MRZ_Y}" width="${W}" height="${H-MRZ_Y}" fill="${MRZ_BG}"/>
    ...
    <text x="${PAD}" y="${MRZ_Y+34}" font-family="monospace" font-size="9.5" fill="${INK}" letter-spacing="1.8">${mrzL1}</text>
    <text x="${PAD}" y="${MRZ_Y+52}" font-family="monospace" font-size="9.5" fill="${INK}" letter-spacing="1.8">${mrzL2}</text>
