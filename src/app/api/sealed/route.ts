...
export async function GET(req: NextRequest) {
  ...
  <g clip-path="url(#cardClip)">
    ...
    <!-- Footer: chain dot + name top-left, tx hash below -->
    <circle cx="${SLEEVE_PAD + 10}" cy="${SLEEVE_PAD + INNER_H + 9}" r="3"
      fill="${chain === 'Solana' ? '#9945FF' : '#0052FF'}"/>
    <text x="${SLEEVE_PAD + 17}" y="${SLEEVE_PAD + INNER_H + 12}"
      font-family="monospace" font-size="6" fill="#666"
      letter-spacing="0.5">${chain.toUpperCase()}</text>
    <text x="${SLEEVE_PAD + 8}" y="${SLEEVE_PAD + INNER_H + 23}"
      font-family="monospace" font-size="6.5" fill="#555"
      letter-spacing="0.5">${uid}</text>

    <!-- Footer: date — shifted left for balance -->
    <text x="${INNER_W * 0.46 + SLEEVE_PAD}" y="${SLEEVE_PAD + INNER_H + 11}"
      font-family="monospace" font-size="5.5" fill="#999"
      letter-spacing="1">ISSUE DATE</text>
    <text x="${INNER_W * 0.46 + SLEEVE_PAD}" y="${SLEEVE_PAD + INNER_H + 22}"
      font-family="monospace" font-size="7" fill="#555"
      letter-spacing="0.5">${dateStr}</text>

    <!-- Footer: seal mark — pulled left from edge -->
