...
const MIN_RATIO = 9 / 16;   // 0.5625 — tall portrait
const MAX_RATIO = 16 / 9;   // 1.778  — wide landscape
...
export async function GET(req: NextRequest) {
  ...
  // Client can pass known dimensions to avoid server-side image decode
  ...
  if (imageUrl) {
    try {
      ...
      if (res.ok) {
        ...
        // Determine ratio — use passed params if available, else parse PNG header
  ...
  // Compute dynamic inner height from ratio
  ...
  <g clip-path="url(#cardClip)">
    ...
    <!-- Footer: seal mark -->
    <g transform="translate(${W - SLEEVE_PAD - 22}, ${FOOTER_Y + 14})">
      ${SEAL_MARK_SVG}
    </g>
