...
export async function POST(req: NextRequest) {
  ...
  if (pending.claimType !== 'website_app_delivery') {
  ...
  try {
    ...
    const result = await verifyWebsiteAppDelivery({
    ...
    const achieved = await attestAchievement({
      ...
      metric:        `perf=${result.evidence.rawMetrics.performance}/100 · a11y=${result.evidence.rawMetrics.accessibility}/100`,
      score,
      onTime,
    });
