...
async function fetchAlchemyNFTs(network: string, wallet: string, chain: 'base' | 'ethereum'): Promise<NFTItem[]> {
  if (!ALCHEMY_KEY) return [];
  try {
    const url = `https://${network}.g.alchemy.com/nft/v3/${ALCHEMY_KEY}/getNFTsForOwner?owner=${wallet}&withMetadata=true&pageSize=50`;
...
export async function GET(req: NextRequest) {
  ...
  if ((chain === 'all' || chain === 'base') && baseWallet) {
    const baseNFTs = await fetchAlchemyNFTs('base-mainnet', baseWallet, 'base');
    results.push(...baseNFTs);
  }

  if ((chain === 'all' || chain === 'ethereum') && baseWallet) {
    const ethNFTs = await fetchAlchemyNFTs('eth-mainnet', baseWallet, 'ethereum');
    results.push(...ethNFTs);
  }
