...
// The session cookie value is a hex token generated by /api/admin/auth using
// Web Crypto (HMAC-SHA256). We verify it here using the same Web Crypto API
// which IS available in the Edge Runtime (unlike Node's 'crypto' module).
...
async function isValidAdminSession(req: NextRequest): Promise<boolean> {
  ...
  // Re-derive the expected token for the current and previous hour
  // (same logic as /api/admin/auth/route.ts generateSessionToken)
...
export async function middleware(req: NextRequest): Promise<NextResponse> {
  ...
  // Admin route gate — check BEFORE serving the page
  if (pathname === '/admin' || pathname.startsWith('/admin/')) {
    const valid = await isValidAdminSession(req);
    if (!valid) {
      const redirectUrl = new URL('/', req.url);
      return addSecurityHeaders(NextResponse.redirect(redirectUrl, { status: 302 }));
    }
  }
