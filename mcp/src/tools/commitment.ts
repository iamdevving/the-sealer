// src/tools/commitment.ts
import { McpServer } from '@modelcontextprotocol/sdk/server/mcp.js';
import { z } from 'zod';
import { sealerFetch, formatError } from '../services/api.js';
import { SEALER_BASE_URL, DIFFICULTY_PARAMS } from '../constants.js';
import type { DifficultyPreviewResponse } from '../types.js';
import { VALID_CLAIM_TYPES } from '../types.js';

export function registerCommitmentTools(server: McpServer): void {
  ...
  - "What's the difficulty for 90 PageSpeed score?" →
    ...
    async ({ claim_type, thresholds }) => {
      try {
        ...
        const validKeys = DIFFICULTY_PARAMS[claim_type] || [];
        for (const [key, value] of Object.entries(thresholds)) {
          params[key] = value;
        }

        const data = await sealerFetch<DifficultyPreviewResponse>(
          '/api/difficulty-preview',
          { params }
        );
        ...
        if (data.bootstrapped) lines.push('⚠️ Bootstrapped estimate — fewer than 50 real data points for this category');
        lines.push('');
  ...
  - "What do I sign to attest a statement?" →
    agent_id='0xYourWallet', action='attest'`,
      inputSchema: z.object({
        ...
        action: z.enum(['attest', 'attest-commitment', 'attest-amendment', 'claim-handle'])
  ...
  - "How is code delivery scored?" → claim_type='code_software_delivery'`,
    ...
    async ({ claim_type }) => {
      ...
      const PARAM_DETAILS: Record<string, {
        label: string;
        metrics: Array<{ key: string; description: string; weight: number; inverted?: boolean; p50: number; p90: number }>;
      ...
      for (const m of detail.metrics) {
        ...
        if (m.inverted) lines.push(`    ⚠️ Inverted: lower value = harder commitment`);
