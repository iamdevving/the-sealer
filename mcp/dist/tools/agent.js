...
function registerAgentTools(server) {
    server.registerTool('sealer_get_agent_profile', {
...
Error Handling:
  - Returns "Agent not found" if handle/wallet has no protocol activity`,
        inputSchema: zod_1.z.object({
            handle_or_wallet: zod_1.z.string()
                .min(1)
        ...
        try {
            const data = await (0, api_js_1.sealerFetch)(`/api/agent/${encodeURIComponent(handle_or_wallet)}`);
            const lines = [];
            ...
            if (data.sid) {
                ...
                if (data.sid.renewalCount > 0)
                    lines.push(`  Renewals: ${data.sid.renewalCount}`);
            }
            lines.push('');
            lines.push('**Stats**');
    ...
    server.registerTool('sealer_get_commitment_status', {
...
Error Handling:
  - Returns 404 if UID not found (commitment may not be registered or may have expired from cache)`,
        ...
        try {
            const data = await (0, api_js_1.sealerFetch)(`/api/commitment/${uid}`);
            ...
            if (data.difficulty !== null)
                lines.push(`Difficulty: ${data.difficulty}/100`);
            if (data.proofPoints !== null)
                lines.push(`Proof Points: ${data.proofPoints}`);
            if (data.amended)
                lines.push(`Amended: Yes`);
            if (data.mintedAt)
                lines.push(`Created: ${new Date(data.mintedAt).toLocaleString()}`);
            if (data.deadline)
                lines.push(`Deadline: ${new Date(data.deadline).toLocaleString()}`);
            if (data.lastChecked)
                lines.push(`Last Checked: ${new Date(data.lastChecked).toLocaleString()}`);
            lines.push('');
            lines.push(`EAS Explorer: ${constants_js_1.EAS_EXPLORER_URL}/${uid}`);
            return {
    ...
    server.registerTool('sealer_check_handle', {
...
Examples:
  ...
  - "Check if sealer.agent is taken" → handle='sealer.agent'`,
        inputSchema: zod_1.z.object({
            handle: zod_1.z.string().optional().describe("Handle to check (e.g. 'aria.agent')"),
            wallet: zod_1.z.string().optional().describe('EVM wallet address (0x...) to look up handle for'),
        }).strict().refine((data) => data.handle !== undefined || data.wallet !== undefined, { message: 'At least one of handle or wallet must be provided' }),
        annotations: {
            readOnlyHint: true,
            destructiveHint: false,
            idempotentHint: true,
        ...
        try {
            ...
            const data = await (0, api_js_1.sealerFetch)('/api/sid/check', { params });
            if (wallet) {
